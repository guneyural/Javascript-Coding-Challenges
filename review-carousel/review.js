import React, { useEffect, useState } from "react";
import Data from "./data";

const Review = () => {
  const [review, setReview] = useState({});
  const [index, setIndex] = useState(0);

  useEffect(() => {
    setReview(Data[index]);
  }, [index]);

  const handleRightClick = () => {
    if (index < Data.length - 1) {
      setIndex(index + 1);
    } else {
      setIndex(0);
    }
  };

  const handleLeftClick = () => {
    if (index > 0) {
      setIndex(index - 1);
    } else {
      setIndex(Data.length - 1);
    }
  };

  const randomClick = () => {
    const randNumber = Math.floor(Math.random() * Data.length);
    setIndex(randNumber);
  };

  return (
    <>
      <div className="review-container">
        <h1 className="tour-header">Our Reviews</h1>
        <div className="bottom-line"></div>
        <div className="review-card" data-id={review.id}>
          <section className="review-card-top">
            <section className="img-circle">
              <svg
                width="1em"
                height="1em"
                viewBox="0 0 16 16"
                className="bi bi-chat-quote-fill quote-icon"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  fillRule="evenodd"
                  d="M16 8c0 3.866-3.582 7-8 7a9.06 9.06 0 0 1-2.347-.306c-.584.296-1.925.864-4.181 1.234-.2.032-.352-.176-.273-.362.354-.836.674-1.95.77-2.966C.744 11.37 0 9.76 0 8c0-3.866 3.582-7 8-7s8 3.134 8 7zM7.194 6.766c.087.124.163.26.227.401.428.948.393 2.377-.942 3.706a.446.446 0 0 1-.612.01.405.405 0 0 1-.011-.59c.419-.416.672-.831.809-1.22-.269.165-.588.26-.93.26C4.775 9.333 4 8.587 4 7.667 4 6.747 4.776 6 5.734 6c.271 0 .528.06.756.166l.008.004c.169.07.327.182.469.324.085.083.161.174.227.272zM11 9.073c-.269.165-.588.26-.93.26-.958 0-1.735-.746-1.735-1.666 0-.92.777-1.667 1.734-1.667.271 0 .528.06.756.166l.008.004c.17.07.327.182.469.324.085.083.161.174.227.272.087.124.164.26.228.401.428.948.392 2.377-.942 3.706a.446.446 0 0 1-.613.01.405.405 0 0 1-.011-.59c.42-.416.672-.831.81-1.22z"
                />
              </svg>
              <img
                src={review.image}
                style={{ objectFit: "cover" }}
                alt={`Face of ${review.name}`}
                className="review-img"
              />
            </section>
          </section>
          <section className="review-card-body">
            <p className="review-name">{review.name}</p>
            <p className="review-job">{review.job}</p>
            <p className="review-description">{review.text}</p>
          </section>
          <section className="review-card-bottom">
            <div className="directions">
              <span
                className="review-slider-left"
                onClick={() => handleLeftClick()}
              >
                &lt;
              </span>
              <span
                className="review-slider-right"
                onClick={() => handleRightClick()}
              >
                &gt;
              </span>
            </div>
            <button className="random-btn" onClick={() => randomClick()}>
              Surprise Me
            </button>
          </section>
        </div>
      </div>
    </>
  );
};

export default Review;
