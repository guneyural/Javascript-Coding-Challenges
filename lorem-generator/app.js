...
const paraConut = document.querySelector("input[type='number']");
const btn = document.querySelector("input[type='submit']")
const paras = document.querySelector(".paragraphs");

btn.addEventListener('click', () => {
     let countOfParagraphs = paraConut.value == "" ? 0 : paraConut.value;
     
     paras.querySelectorAll('p').forEach(item => {
          item.remove();
     });

     if(countOfParagraphs == 0) {
          let randNumber = Math.floor(Math.random(0)*text.length);
          let textData = text[randNumber];

          const p = document.createElement('p');
          p.innerText = textData;
          paras.appendChild(p);
     } else {
          for(let i = 0; i< countOfParagraphs; i++) {
               const p = document.createElement('p');
               p.innerText = text[i];
               paras.appendChild(p);
          }
